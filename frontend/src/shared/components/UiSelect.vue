...
.ui-select__trigger:hover:not(:disabled),.ui-select--open .ui-select__trigger { border-color:var(--color-border-strong); background-color:#252d38; }.ui-select__trigger:disabled { cursor:not-allowed; opacity:.45; }
.ui-select__menu { width:max-content; overflow:auto; padding:5px; border:1px solid var(--color-border-strong); border-radius:var(--radius-control); background:var(--color-surface-raised); box-shadow:0 12px 30px rgba(0,0,0,.52); color:var(--color-text); }
...
.ui-select__option>span { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.ui-select__option svg { flex:none; color:var(--color-primary); }.ui-select__option.active { background:#2a3441; }.ui-select__option.selected { color:#fff; background:rgba(77,141,255,.18); }.ui-select__option:disabled { opacity:.42; }
